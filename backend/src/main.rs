...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let oidc = if args.disable_oidc {
        tracing::warn!("OIDC authentication is DISABLED — do not use in production");
        OidcState::Disabled
    } else {
        let oidc_config = config
            .oidc
            .expect("--disable-oidc not set but [oidc] section is missing from config");

        let http_client = &reqwest::Client::new();
        let oidc_provider_metadata = CoreProviderMetadata::discover_async(
            IssuerUrl::new(oidc_config.provider_url)?,
            http_client,
        )
        .await
        .expect("Failed to discover provider metadata");

        let authorization_endpoint = oidc_provider_metadata.authorization_endpoint().to_string();
        let token_endpoint = oidc_provider_metadata
            .token_endpoint()
        ...
        let allowed_subs: HashSet<String> = if !oidc_config.allowed_subs.is_empty() {
            oidc_config.allowed_subs.into_iter().collect()
        } else {
            std::env::var("OIDC_ALLOWED_SUBS")
                .unwrap_or_default()
                .split(',')
                .filter(|s| !s.is_empty())
